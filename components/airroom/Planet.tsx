...
export function Planet() {
  ...
  const [hud, setHud] = useState({ crumb: "from orbit · the whole now", alt: "orbit", hearing: "the hum of the whole now" })

  const verifiedRef = useRef(false)
  const inCallRef = useRef(false)
  useEffect(() => { verifiedRef.current = verified }, [verified])
  useEffect(() => { inCallRef.current = !!selected; try { setAmbienceMuted(!!selected) } catch { /* */ } }, [selected])
  ...
  const confirm18 = () => {
    ...
    const p = pending; setPending(null); if (p) setSelected(p)
  }
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      const sig = crumb + "|" + altl + "|" + hear
      if (sig !== lastHud) { lastHud = sig; setHud({ crumb, alt: altl, hearing: hear }) }
    }
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#04050b", overflow: "hidden", touchAction: "none" }}>
      ...
      <div style={{ position: "absolute", left: 16, bottom: "calc(env(safe-area-inset-bottom) + 16px)", fontSize: 12.5, color: "#cfe0ee", background: "rgba(4,5,11,.55)", padding: "8px 13px", borderRadius: 12, maxWidth: "76%", pointerEvents: "none", fontFamily: "var(--font-geist), system-ui, sans-serif" }}>{hud.hearing}</div>
        ...
        <div style={{ position: "absolute", inset: 0, background: "rgba(20,6,30,.9)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 26, zIndex: 30 }}>
          <div style={{ maxWidth: 340, textAlign: "center", color: "#f3e8fb" }}>
            ...
            <div style={{ marginTop: 18, display: "flex", flexDirection: "column", gap: 9 }}>
              ...
              <button onClick={() => setPending(null)} style={{ fontSize: 14, color: "#d7c3ea", background: "transparent", border: ".5px solid rgba(198,156,255,.3)", borderRadius: 14, padding: "12px 0", cursor: "pointer" }}>keep me up here</button>
            </div>
          </div>
        </div>
      )}
