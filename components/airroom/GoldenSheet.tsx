...
export function GoldenSheet({ onClose }: { onClose: () => void }) {
  ...
  const buy = async () => {
    ...
    if (!r.ok) { setBusy(false); setNote(r.error || "checkout didn't open") }
  }

  return (
    <div role="dialog" aria-label="the golden room" onClick={onClose}
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: 11.5, letterSpacing: 1.6, textTransform: "uppercase", fontWeight: 700,
            background: METAL, WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent" }}>the golden room</span>
        ...
        {held > 0 && (
          <p style={{ ...panel(), margin: "0 0 14px", padding: "11px 13px", fontSize: 13.5 }}>
            you&apos;re holding <strong style={{ color: GOLD }}>{held}</strong> — open one from inside any conversation.
          </p>
        )}
