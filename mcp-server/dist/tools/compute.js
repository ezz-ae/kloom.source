...
Returns:
  ...
  - label: Context label if provided`,
        ...
        if (/[^0-9+\-*/.()\s%*^]/.test(expression.replace(/\*\*/g, "").replace(/e[+-]\d+/g, ""))) {
            return { content: [{ type: "text", text: "Invalid expression — only numbers and math operators (+, -, *, /, **, %, ()) are allowed." }] };
        }
        try {
            // eslint-disable-next-line no-new-func
            const result = Function(`"use strict"; return (${expression})`)();
            if (typeof result !== "number" || !isFinite(result)) {
                return { content: [{ type: "text", text: "Result is not a finite number." }] };
            }
...
Args:
  ...
  - timeframe (string): Analysis horizon — scalp | swing | position`,
        ...
        try {
            const res = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${coinId}&vs_currencies=usd&include_24hr_change=true&include_market_cap=true&include_24hr_vol=true`, { signal: AbortSignal.timeout(6000) });
            ...
            return { content: [{ type: "text", text: `Market analysis failed: ${err.message}` }] };
        }
    });
}
