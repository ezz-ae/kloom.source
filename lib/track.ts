 ...
 *   initiate_checkout — they tapped pay and we sent them to Ziina (value known)
 *   purchase          — they paid (value + currency) ← the one that matters for ROAS
 *   lead              — they left an email (AIRRAW)
 *
 * initiate_checkout is the dense mid-funnel signal a brand-new, low-AOV pixel can
 * actually optimize on while real Purchase volume is still too sparse to exit the
