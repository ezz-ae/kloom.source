...
import {
  Mic, Users, Sparkles, Shield, Zap, Globe2,
  ChevronRight, Check, Bot, ArrowRight,
  User, UsersRound, Crown, Infinity as InfinityIcon, EyeOff, Rocket,
} from "lucide-react"

// ── The galaxy: every planet is its own world ──────────────────────────────────
const PLANETS = CATEGORY_ORDER.map((c) => CATEGORY_META[c])

// ── Two ways to land: alone, or with a crew (the invite none/one/many idea) ─────
const MODES = [
  {
    icon: User,
    title: "Land solo",
    accent: "text-sky-300 bg-sky-500/10 border-sky-500/25",
    body: "One-on-one. Just you and the cast — a private planet that's yours alone, remembers you, and never leaves a trace you didn't ask for.",
    tag: "1-on-1",
  },
  {
    icon: UsersRound,
    ...
    body: "Beam your partner, your co-founder or the whole group onto the same planet with one link. Voice and chat, live, everyone in the same orbit.",
    tag: "Invite many",
  },
]
...
// ── How it works ──────────────────────────────────────────────────────────────
const STEPS = [
  { n: "01", title: "Pick a planet", body: "Eleven worlds, each with its own gravity, cast and rules — the trading floor, the dark side, deep space, the build planet. Every door opens somewhere different." },
    ...
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      <section className="relative flex flex-col items-center justify-center min-h-screen text-center px-4 pt-20">
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {/* Animated nebula blobs — lightweight, no WebGL, no console noise */}
        ...
        <div className="relative z-10 max-w-3xl mx-auto space-y-6">
          <div className="inline-flex items-center gap-2 bg-foreground/5 border border-border rounded-full px-4 py-1.5 text-xs font-medium text-foreground/70">
            ...
            11 planets · 100+ characters · Claude · Gemini · GPT · free to chat
          </div>
      ...
      <section id="planets" className="max-w-6xl mx-auto px-6 py-24">
        <div className="text-center mb-12">
          ...
          <h2 className="text-4xl sm:text-5xl font-black tracking-tight leading-tight">
            Each planet is totally its own.
          </h2>
          <p className="text-foreground/55 mt-4 max-w-2xl mx-auto text-base leading-relaxed">
            Different gravity, different cast, different rules. Pick where you want to be tonight —
            then make it yours.
          </p>
        </div>
      ...
      <section id="modes" className="max-w-5xl mx-auto px-6 py-20">
        <div className="text-center mb-12">
          ...
          <h2 className="text-4xl sm:text-5xl font-black tracking-tight leading-tight">
            Go alone. Or bring everyone.
          </h2>
          <p className="text-foreground/55 mt-4 max-w-2xl mx-auto text-base leading-relaxed">
            Every planet works both ways — a private one-on-one that's only yours, or an open
            orbit where your whole crew lands on the same link.
          </p>
        </div>
          ...
          {["launch-war-room", "build-studio", "growth-boardroom"]
            ...
            .map((r) => (
              <Link key={r.id} href={`/app/rooms/${r.id}`}
                ...
                <span className="inline-flex items-center gap-1.5 text-emerald-300 text-sm font-semibold mt-4 group-hover:gap-2.5 transition-all">
      ...
      <section id="passes" className="max-w-5xl mx-auto px-6 py-24">
        <div className="text-center mb-12">
          ...
          <p className="text-foreground/60 text-lg mt-4 max-w-xl mx-auto leading-relaxed">
            Every character is free to text, forever. Pay only when you want to go bigger —
            live voice, the premium models, and the planets behind the velvet rope.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-4xl mx-auto">
          ...
          <div className="rounded-3xl border border-border bg-foreground/[0.03] p-6 flex flex-col">
            ...
            <Link href="/app/rooms" className="mt-6 text-center text-sm font-bold py-3 rounded-xl bg-foreground/[0.06] hover:bg-foreground/10 transition-colors">
              Start free
            </Link>
          </div>
          ...
          <div className="rounded-3xl border border-amber-500/30 bg-gradient-to-br from-amber-900/20 to-stone-950 p-6 flex flex-col relative">
            ...
            <Link href="/app/rooms" className="mt-6 text-center text-sm font-bold py-3 rounded-xl bg-amber-500 text-stone-950 hover:bg-amber-400 transition-colors">
              Go live
            </Link>
          </div>
          ...
          <div className="rounded-3xl border border-fuchsia-500/30 bg-gradient-to-br from-fuchsia-900/20 to-stone-950 p-6 flex flex-col">
            ...
            <Link href="/app/rooms" className="mt-6 text-center text-sm font-bold py-3 rounded-xl bg-fuchsia-500 text-foreground hover:bg-fuchsia-400 transition-colors">
              Unlock everything
            </Link>
          </div>
        </div>
      ...
      <section className="max-w-4xl mx-auto px-6 py-20">
        <div className="rounded-[2rem] border border-border bg-gradient-to-br from-stone-900 via-stone-950 to-fuchsia-950/20 p-10 sm:p-14 text-center">
          ...
          <p className="text-foreground/55 text-lg sm:text-xl mt-6 max-w-xl mx-auto leading-relaxed">
            Claude, Gemini and GPT think out loud together — and community-built planets mean
            the best worlds are always one click away. What you build here stays yours.
          </p>
        </div>
      </section>
      ...
      <section className="max-w-5xl mx-auto px-6 py-20">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          {[
            ...
            { icon: Globe2, title: "Voice in 50+ languages", body: "Natural real-time calls that interrupt and respond like a person." },
          ].map((f) => (
            <div key={f.title} className="text-center">
              <div className="w-12 h-12 rounded-xl bg-foreground/5 border border-border flex items-center justify-center mx-auto mb-4">
                <f.icon size={20} className="text-fuchsia-400" />
              </div>
