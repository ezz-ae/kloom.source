  ...
  - timeframe: Analysis window`,
    ...
    async ({ address, timeframe }) => {
      try {
        ...
        if (!pair) return { content: [{ type: "text" as const, text: `Token ${address.slice(0,8)}… not found on DexScreener.` }] }
        ...
        return { content: [{ type: "text" as const, text: `Chart analysis failed: ${(err as Error).message}` }] }
      }
    }
  )
...
Args:
  ...
  - inputs: Key-value pairs of inputs`,
    ...
    async ({ calc_type, inputs }) => {
      ...
      return {
        content: [{ type: "text" as const, text: JSON.stringify({ calc_type, inputs, result, widget }, null, 2) }],
