        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between gap-4 mb-4">
            <div>
              ...
              <p className="text-sm text-white/40 mt-0.5">{EXPERTS.length} specialists · voice + chat · real expertise</p>
            </div>
            ...
            {ALL_GROUPS.map((g) => (
              <button key={g} onClick={() => setGroup(g)}
                className={`shrink-0 px-3.5 py-1.5 rounded-full text-xs font-semibold border transition-all ${
                  group === g ? "bg-white text-stone-950 border-transparent" : "bg-white/5 border-white/10 text-white/50 hover:bg-white/10 hover:text-white"
                }`}>
            ...
            {filtered.map((e) => (
              <div key={e.id}
                ...
                <div className="flex items-start gap-3">
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-amber-500/30 to-orange-500/30 border border-white/10 flex items-center justify-center text-2xl shrink-0">
                    {e.emoji}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap mt-1">
                      <span className="text-[11px] text-white/45">{e.name}</span>
                ...
                <p className="text-xs text-white/45 italic leading-relaxed line-clamp-2">{e.tagline}</p>
...
export default function ExpertsPage() {
  return <Suspense fallback={<div className="min-h-full bg-stone-950" />}><ExpertsContent /></Suspense>
}
