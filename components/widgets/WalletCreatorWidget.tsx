      ...
      <div className="p-4 space-y-4">
        {!kp ? (
          <div className="text-center space-y-3">
            <p className="text-sm text-foreground/50">Generate a fresh Solana wallet keypair. The private key is created locally — never sent anywhere.</p>
            <button onClick={generate}
              className="bg-amber-500 hover:bg-amber-400 text-foreground font-bold px-6 py-2.5 rounded-xl transition-all hover:scale-[1.02] text-sm">
              Generate wallet
            </button>
          </div>
        ) : (
          <div className="space-y-4">
            {/* Public key */}
            <div>
              <div className="text-[10px] uppercase tracking-widest text-foreground/30 font-bold mb-1.5">Public address</div>
              <div className="flex items-center gap-2 bg-white/5 border border-border/50 rounded-xl px-3 py-2.5">
                <code className="text-xs font-mono text-emerald-300 flex-1 truncate">{kp.pub}</code>
                <button onClick={() => copy("pub")} className="text-foreground/40 hover:text-foreground/80 shrink-0">
            ...
            {/* Private key */}
            <div>
              <div className="text-[10px] uppercase tracking-widest text-foreground/30 font-bold mb-1.5 flex items-center gap-1.5">
              ...
              <div className="flex items-center gap-2 bg-amber-500/5 border border-amber-500/20 rounded-xl px-3 py-2.5">
                ...
                <button onClick={() => setShow((v) => !v)} className="text-foreground/40 hover:text-foreground/80 shrink-0">
                  {showPrivate ? <EyeOff size={13} /> : <Eye size={13} />}
                </button>
                <button onClick={() => copy("priv")} className="text-foreground/40 hover:text-foreground/80 shrink-0">
            ...
            <label className="flex items-start gap-2 cursor-pointer">
              ...
              <span className="text-xs text-foreground/50">I have saved my private key securely in a password manager or hardware wallet</span>
            </label>
