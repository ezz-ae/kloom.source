  ...
  - goals: What the creator wants to achieve (optional, gathered in conversation)`,
    ...
    async ({ url, goals }) => {
      ...
      const detect = (u: string) => {
        ...
        if (m.includes("instagram.com"))  return { platform: "Instagram", handle: u.split("instagram.com/")[1]?.split(/[/?]/)[0] }
        if (m.includes("tiktok.com"))     return { platform: "TikTok",    handle: u.split("@")[1]?.split(/[/?]/)[0] }
        if (m.includes("onlyfans.com"))   return { platform: "OnlyFans",  handle: u.split("onlyfans.com/")[1]?.split(/[/?]/)[0] }
        if (m.includes("youtube.com") || m.includes("youtu.be")) return { platform: "YouTube", handle: u.split(/[@/]/).pop() }
        if (m.includes("twitter.com") || m.includes("x.com"))    return { platform: "X (Twitter)", handle: u.split(".com/")[1]?.split(/[/?]/)[0] }
