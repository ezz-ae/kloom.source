...
async function fetchCoinGecko(ids, extras = "") {
...
Returns: Array of price objects, same schema as ora_get_crypto_price`,
        ...
        try {
            ...
            const results = symbols.map((s) => {
                ...
                const coin = data[coinId];
                if (!coin)
                    return { symbol: s.toUpperCase(), error: "not found" };
                const change = Number(coin.usd_24h_change ?? 0);
                return {
...
Args:
  - identifier (string): Token name, symbol, or contract/mint address`,
        ...
        try {
            const searchRes = await fetch(`https://api.coingecko.com/api/v3/search?query=${encodeURIComponent(identifier)}`, { signal: AbortSignal.timeout(6000) });
            const searchData = await searchRes.json();
            const coin = searchData.coins?.[0];
            if (!coin) {
                return { content: [{ type: "text", text: `Token "${identifier}" not found on CoinGecko.` }] };
            }
            ...
            return { content: [{ type: "text", text: `Token lookup failed: ${err.message}` }] };
        }
    });
}
