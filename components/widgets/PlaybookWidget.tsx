...
const PLAYBOOKS: Record<string, Playbook> = {
  "token-launch": {
    ...
    category: "trading",
    steps: [
      { id: "1", title: "Define tokenomics",           description: "Total supply, allocation, vesting, unlock schedule",           action: "Use kloom_calculate for exact numbers" },
      { id: "2", title: "Audit smart contract",        description: "Run security audit before deployment",                        action: "Use kloom_analyze_code on your contract" },
  ...
  "trading-strategy": {
    ...
    category: "trading",
    steps: [
      { id: "1", title: "Market regime check",         description: "Is this a trending or ranging market? Never fight the regime",  action: "Use kloom_analyze_market" },
      ...
      { id: "4", title: "Position sizing",             description: "Risk X% of portfolio. Never more. Calculate size.",            action: "Use kloom_calculate: (portfolio × risk%) ÷ (entry − stop)" },
      ...
      { id: "6", title: "R:R minimum 2:1",             description: "If you can't get 2:1, skip the trade.",                       action: "Use kloom_calculate to verify" },
    ...
    <div className="rounded-2xl border border-border/50 bg-stone-900 overflow-hidden my-1">
      <div className="px-4 py-3 border-b border-white/8 bg-white/5">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="ml-auto text-xs text-foreground/40">{done.size}/{pb.steps.length}</span>
        </div>
        <p className="text-[11px] text-foreground/40">{pb.description}</p>
        {/* Progress bar */}
        ...
        {pb.steps.map((step, i) => (
          <div key={step.id} className={`${done.has(step.id) ? "opacity-50" : ""}`}>
            <div className="flex items-start gap-3 px-4 py-3">
              ...
              >
                {done.has(step.id) && <Check size={10} className="text-foreground" />}
              </button>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-foreground/25 font-mono">{String(i+1).padStart(2,"0")}</span>
                  <span className={`text-sm font-semibold ${done.has(step.id) ? "line-through text-foreground/30" : ""}`}>{step.title}</span>
                  <button onClick={() => toggleExpand(step.id)} className="ml-auto text-foreground/30 hover:text-foreground/60">
      ...
      <div className="px-4 py-3 border-t border-white/8 bg-white/3 flex flex-wrap gap-2">
        <span className="text-[10px] text-foreground/25 self-center">Other playbooks:</span>
