      ...
      <div style={{ width: "100%", maxWidth: 360, background: "linear-gradient(180deg,#0d1018,#070910)", border: ".5px solid rgba(255,255,255,.12)", borderRadius: 20, padding: "26px 22px", textAlign: "center", color: "#eef4f8" }}>
        {state === "done" ? (
          <>
            ...
            <div style={{ fontSize: 19, fontWeight: 600 }}>{t("you're in.")}</div>
            <div style={{ fontSize: 13, color: "#9fb2c4", marginTop: 6 }}>{t("founding access saved. we'll call you when the floor opens.")}</div>
          </>
        ) : (
          <>
            ...
            <input
              ...
              type="email" inputMode="email" autoComplete="email" placeholder={t("you@email.com")}
            ...
            />
            {state === "error" && <div style={{ fontSize: 11.5, color: "#ff9c73", marginTop: 7 }}>{t("that email looks off — try again?")}</div>}
            ...
            <button onClick={onClose} style={{ marginTop: 10, fontSize: 12.5, color: "#7f93a5", background: "transparent", border: "none", cursor: "pointer" }}>{t("maybe later")}</button>
          </>
        )}
      </div>
    </div>
